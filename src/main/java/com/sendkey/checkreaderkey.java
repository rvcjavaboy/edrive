...
/**
 * Servlet implementation class checkreaderkey
 */
@WebServlet("/Account/reader/checkreaderkey")
public class checkreaderkey extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public checkreaderkey() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(b){
			...
			response.sendRedirect(response.encodeURL("../../downloadfile")+"?filecontext="+request.getParameter("reqest_id")+"&my_type_file=reader&key="+request.getParameter("key")+"");

		}else{
			response.sendRedirect("/Account/reader/secret_key.jsp?reqest_id="+request.getParameter("reqest_id")+"&key_status=Key Is Wrong");
