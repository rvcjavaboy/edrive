...
/**
 * Servlet implementation class checkwriterkey
 */
@WebServlet({ "/checkwriterkey", "/Account/writer/checkwriterkey" })
public class checkwriterkey extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public checkwriterkey() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(b){
			...
			response.sendRedirect(response.encodeURL("../../downloadfile")+"?filecontext="+request.getParameter("reqest_id")+"&my_type_file=writer&key="+request.getParameter("key")+"");

		}else{
			response.sendRedirect("/Account/writer/secret_key.jsp?reqest_id="+request.getParameter("reqest_id")+"&key_status=Key Is Wrong");
