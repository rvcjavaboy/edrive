...
public class downloadfile extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		}else if(request.getParameter("my_type_file").equals("reader")){
			String name=(String)session.getAttribute("r_email");
			String pass=(String)session.getAttribute("r_pass");
			String type=(String)session.getAttribute("r_type");

			ses=com.account.session.mgm.UserSession.getSessionAvaliable(name, pass, type);
			
		}else if(request.getParameter("my_type_file").equals("writer")){
			String name=(String)session.getAttribute("w_email");
		...
		try{
			...
			ResultSet rst =st.executeQuery("select * from files where id="+Integer.parseInt(request.getParameter("filecontext"))+" AND keyss='"+request.getParameter("key")+"'");
			if(rst.next()){
